...
function SingleMovie() {
  ...
  useEffect(() => {
    axiosInstance.get(`/movie/${id}?api_key=${API_KEY}`).then(res => setSingleData(res.data)).catch(console.error)
  }, [id, axiosInstance])

  useEffect(() => {
    axiosInstance.get(`/movie/${id}/videos?api_key=${API_KEY}`).then(res => setVideos(res.data.results.slice(0, 5))).catch(console.error)
  }, [id, axiosInstance])

  useEffect(() => {
    axiosInstance.get(`/movie/${id}/videos?api_key=${API_KEY}`).then(res => setVideosMore(res.data.results.slice(5, 10))).catch(console.error)
  }, [id, axiosInstance])

  useEffect(() => {
    axiosInstance.get(`/movie/${id}/credits?api_key=${API_KEY}`).then(res => setActors(res.data.cast)).catch(console.error)
  }, [id, axiosInstance])
